...
//  FlexAccess
//
//  UDP 4992 VITA-49 discovery broadcast listener + SmartLink radio injection.
//  Uses BSD sockets with SO_REUSEPORT + SO_BROADCAST so multiple apps can
//  coexist on the same port (SmartSDR for Mac, MacLogger, etc.).
//
...
final class FlexDiscovery: ObservableObject {
        ...
        let t = Thread {
            ...
            while true {
                ...
                if n > 0 {
                    ...
                    DispatchQueue.main.async { [weak self] in self?.parsePacket(slice, count: n) }
                } else if n == 0 {
                    continue
                } else {
                    let err = errno
                    if err == EINTR { continue }
                    AppFileLogger.shared.log("FlexDiscovery: recv error \(err)")
                    break
                }
            }
    ...
    func injectSmartLinkRadio(_ radio: DiscoveredRadio) { upsert(radio, stale: false) }
    ...
    private func parsePacket(_ buf: [UInt8], count: Int) {
        ...
        guard be32(buf, at: 4) == 0x00000800 else { return }
        if classIDPres && count >= 16 {
            let oui = be32(buf, at: 8) & 0x00FFFFFF
            guard oui == 0x001C2D else { return }
        }
        var hdrWords = 2
        if classIDPres { hdrWords += 2 }
        if tsi != 0    { hdrWords += 1 }
        if tsf != 0    { hdrWords += 2 }
    ...
    private func parseDiscoveryPayload(_ text: String) {
        ...
        for token in text.split(separator: " ", omittingEmptySubsequences: true) {
            ...
            if parts.count == 2 { kv[String(parts[0]).lowercased()] = String(parts[1]) }
        }
    ...
    private func upsert(_ radio: DiscoveredRadio, stale: Bool) {
        if let idx = radios.firstIndex(where: { $0.id == radio.id }) {
            ...
            radios.append(radio)
            AppFileLogger.shared.log("FlexDiscovery: found \(radio.displayName)")
        }
